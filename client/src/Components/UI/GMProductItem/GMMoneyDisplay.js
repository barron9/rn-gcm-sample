...
export default GMMoneyDisplay = (props) => {
    ...
    return (
        <View style={{ flexDirection: 'row', margin: 5, alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 20, fontWeight: 'bold', color: '#555',marginLeft:1 }}>{props.currencytype}</Text>

                <Text style={{ fontSize: 20, fontWeight: 'bold', color: '#333' }}>{moneydisplay(props).split(",")[0]}
                </Text>
                <Text style={{ fontSize: 15, fontWeight: 'bold', color: '#333' }}>{props.price > 999 ? "," : ""}</Text>
                <Text style={{ fontSize: 15, fontWeight: 'bold', color: '#333' }}>{moneydisplay(props).split(",")[1]}
                </Text>
            </View>
